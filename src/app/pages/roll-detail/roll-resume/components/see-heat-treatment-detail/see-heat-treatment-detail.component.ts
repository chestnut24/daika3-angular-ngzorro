...
import {AlarmRecordModalComponent} from '../../../../heat-treatment/components/alarm-record-modal/alarm-record-modal.component';
import {DatePipe} from '@angular/common';
import * as echarts from 'echarts';

@Component({
  selector: 'app-see-heat-treatment-detail',
  templateUrl: './see-heat-treatment-detail.component.html',
  styleUrls: ['./see-heat-treatment-detail.component.scss']
})
export class SeeHeatTreatmentDetailComponent implements OnInit, OnDestroy, AfterViewInit  {
  dataDetail: HtDetail;
  deviceId: string;
  nowTime: number;
  timer;
  ...
  initDetail(): void {
    this.heatTreatmentService.getHtHistoryById(this.deviceId).subscribe(data => {
      ...
      if (data['historyData']) {
        const webSocketData = data['historyData'];
    ...
    if (this.startSuperviseHt && this.startSuperviseHt.findRollRorkRecordDtoList
      ...
      this.startSuperviseHt.findRollRorkRecordDtoList.forEach(items => {
        ...
        this.option.series.push({
          ...
          name: '热处理监测',
          type: 'scatter',
          data: [[Number(items['createDt']), Number(items['avgtemp'])]],
          // @ts-ignore
          label: {
            show: true,
            position: items['type'] == 0 ? 'top' : 'bottom',
  ...
  startOrEndMonitor(): void {
    ...
    if (!this.dataDetail.htRecord.startTime) {
      this.heatTreatmentService.startMonitor({
        ...
        monitorStandard: this.dataDetail.htRecord.monitorStandard ? this.dataDetail.htRecord.monitorStandard : MonitorStandard.AVG}).subscribe(data => {
            ...
            if (Object.keys(webSocketData).length) {
              ...
              if (this.furnaceDevices && this.furnaceDevices.length) {
                this.option.series[0] = {
                  name: this.furnaceDevices[this.furnaceIndex].monitorPostion,
                  type: 'line',
                  // @ts-ignore
                  data: this.furnaceDevices[this.furnaceIndex].otherInfo.history.map(item => Number(item.value))
                };
                // @ts-ignore
                this.option.xAxis.data = this.furnaceDevices[this.furnaceIndex].otherInfo.history.map(item => new DatePipe('zh-Hans').transform(Number(item.ts), 'HH:mm:ss'));
              }
              if (this.rollDevices && this.rollDevices.length) {
                this.option.series[1] = {
                  name: this.rollDevices[this.rollIndex].monitorPostion,
                  type: 'line',
                  // @ts-ignore
                  data: this.rollDevices[this.rollIndex].otherInfo.history.map(item => Number(item.value))
                };
                // @ts-ignore
                this.option.xAxis.data = this.rollDevices[this.rollIndex].otherInfo.history.map(item => new DatePipe('zh-Hans').transform(Number(item.ts), 'HH:mm:ss'));
              }
            }
            this.changeZoom();
