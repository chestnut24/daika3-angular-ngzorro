...
import { DeviceProductFormComponent } from './device-product-form/device-product-form.component';
...
@Component({
    selector: 'app-device-product-management',
    templateUrl: './device-product-management.component.html',
    styleUrls: ['./device-product-management.component.scss']
})
export class DeviceProductManagementComponent implements OnInit {
    ...
    titleList: Array<TitleType> = [
        ...
        { name: '设备名称', canSort: false, sortField: 'name', width: '110px' },
        { name: '设备编号', canSort: false, sortField: 'equipmentNumber', width: '75px' },
        { name: '设备类型', canSort: false, sortField: 'productionTypeName', width: '100px' },
        { name: '所在区域', canSort: false, sortField: 'areaName', width: '110px' },
        { name: '关联设备', canSort: false, sortField: 'equipmentName', width: '300px' },
    ...
    constructor(
        public delongTable: DelongTableService<DeviceProductionList>,
    ...
    ngOnInit() {
        ...
        // table-获取数据
        this.delongTable.observeFunction = this.configurationManagement.getProductFindList.bind(this.configurationManagement);
    }
    ...
    delNode(data: string): void {
        this.modalService.create({
            ...
            nzClosable: false,
            nzOnOk: () => {
                this.configurationManagement.delProductionType(Number(data)).subscribe(success => {
    ...
    handleOk(): void {
        if (this.editData.isType) {
            // 类型
            this.configurationManagement.addProductionType(this.editData).subscribe(success => {
            ...
            if (this.editData.id) {
                val.id = this.editData.id;
            }
            val.areaName = this.editData.typeName;
            this.configurationManagement.addProductionArea(val).subscribe(success => {
                this.messageService.create('success', `${this.editData.isAdd ? '添加' : '编辑'}成功`);
                this.getNodeAreaList();
                this.editNodeDialog = false;
            }, error => {
    ...
    // 列表
    // 列表删除
    delListData(data: number) {
        this.modalService.create({
            nzTitle: '提示',
            nzContent: '确认删除该数据吗？',
            nzClosable: false,
            nzOnOk: () => {
                this.configurationManagement.delProduction(data).subscribe(success => {
                    this.messageService.create('success', '删除成功');
                    this.getList();
                }, error => {
                    this.messageService.create('error', `删除失败， ${error.error.message}`);
    ...
    addListData(): void {
        ...
        const modal = this.modalService.create({
            ...
            //     hidePassword
            // },
            nzContent: DeviceProductFormComponent,
            nzFooter: [
                {
                    label: '取消',
                    shape: 'default',
                ...
                },
                {
                    ...
                    onClick: (contentComponentInstance) => {
                        ...
                        // 数据处理
                        const formVal = this.addDataProcessing(cci.dataForm.value);
                        this.configurationManagement.addProduction({
                            name: formVal.name,
                            typeId: formVal.typeid,
                            number: formVal.number,
                            manufacturer: formVal.productionManufacturers,
                            responsibleUser: formVal.responsibleUser,
                            jcUuid: formVal.jcUuid,
                            jcAttribute: formVal.jcAttribute,
                            areaId: formVal.areaid,
                            monitorList: formVal.lftArr,
                        }).subscribe(success => {
    ...
    // 列表修改
    editListData(dataId: DeviceProductionList): void {
        // DeviveProductFormComponent
        // modle 的 服务式引入
        let showload = false;
        const modal = this.modalService.create({
    ...
    // 表单数据处理
    addDataProcessing(data: any): any {
        const lftArr = [];
        data.ListFurnaceTemperature.forEach(item => {
            item.monitorId.forEach(item2 => {
                lftArr.push({
                    monitorPostion: item.monitorName,
                    type: 'FURNACE_TEMPERATURE',
                    id: item2
                });
            });
        });
        data.millRollTemperature.forEach(item => {
            item.monitorId.forEach(item2 => {
                lftArr.push({
                    monitorPostion: item.monitorName,
